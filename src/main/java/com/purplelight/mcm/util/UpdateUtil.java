...
public class UpdateUtil {
	...
	public static<T> T copyNotNullOrEmptyValue(T org, T tar) throws Exception{
		return copyNotNullOrEmptyValue(org, tar, new ArrayList<String>());
	}
	 ...
	 * @param org           原始对象
	 * @param tar           目标对象
	 * @param alwaysUpdateFields     总是更新的字段，不管是否值为null或0
	 * @return              拷贝后的原始对象
	 * @throws Exception    如果原始对象和目标对象的类型不一致，则抛出异常。
	 */
	public static<T> T copyNotNullOrEmptyValue(T org, T tar, List<String> alwaysUpdateFields) throws Exception{
		// 如果有一方为Null，则直接返回。
		...
		if (org.getClass().getName().equals(tar.getClass().getName())){
			...
			for(Method tarM : tarMethods){
				...
				if (tarM.getName().startsWith("get") || tarM.getName().startsWith("is")){
					...
					if (alwaysUpdateFields.contains(cmpName)){
						isNeededToCopy = true;
					} else {
